...
class TransformerModel(torch.nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers, dropout=0.5):
        ...
        self.encoder = torch.nn.Embedding(input_dim, d_model)
    ...
    def forward(self, query, key, value):
        ...
        output = self.transformer_encoder(query, key, value)
        return output
...
class Net(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers):
        ...
        self.transformer = TransformerModel(input_dim, d_model, nhead, num_layers)
    ...
    def forward(self, in_data, out_data):
        ...
        copied_in_x = copy.deepcopy(in_x)
        in_y = in_data.y
        ...
        # Cross-Attention 
        cross_attn_out = self.transformer(q=in_x, k=out_x, v=out_x)
        cross_attn_out.squeeze_(dim=1)
        cross_pred_out = self.prm(cross_attn_out)
        kl_loss =  F.kl_div(F.log_softmax(in_pred, dim=1).log(), F.softmax(cross_pred_out, dim=1), reduction='batchmean')

        all_loss = in_cl_loss + out_cl_loss + cross_cl_loss1 + cross_cl_loss2 + prototype_cl_loss + kl_loss
